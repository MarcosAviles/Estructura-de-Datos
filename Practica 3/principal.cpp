...
int main(int argc, char *argv[]){
    ...
    vector<mutacion>::const_iterator it=conjuntoMutaciones.lower_bound("2",1);
    cout<<"El numero de mutaciones asociadas al cromosoma 1 es:"<<it-conjuntoMutaciones.cbegin()<<endl;

    
    cout<<"--------------------------------------------------------------------"<<endl;

    /**@todo ¿Existe la mutación con ID "rs147165522"? Imprimir la mutación y las enfermedades asociadas */
    pair<conjunto::value_type, bool> aux2=conjuntoMutaciones.find("rs147165522");
    if(aux2.second){
      cout<<"Mutacion con ID rs147165522: --------------------------------------"<<endl;
      cout<<aux2.first<<endl;
      cout<<"-------------------------------------------------------------------"<<endl;
    }
    else{
      cout<<"No existe la mutacion con esa ID"<<endl;
    }
    
    /**@todo ¿Existe la mutación en chr/pos "14"/67769578? Imprimir la mutación y las enfermedades asociadas */
    aux2=conjuntoMutaciones.find("14", 67769578);
    if(aux2.second){
      cout<<"Mutacion con chr/pos '14'/67769578: --------------------------------------"<<endl;
      cout<<aux2.first<<endl;
      cout<<"-------------------------------------------------------------------"<<endl;
    }
    else{
      cout<<"No existe la mutacion con chr/pos '14'/67769578"<<endl;
    }

    /**@todo ¿Cómo podríamos calcular el número de mutaciones del cromosoma 3? (utiliza lower_bound / upper_bound) */
    it=conjuntoMutaciones.lower_bound("2", 1);
    vector<mutacion>::const_iterator it2=conjuntoMutaciones.lower_bound("4",1);
    cout<<"El numero de mutaciones asociadas al cromosoma 3 es:"<<it2-it<<endl;
