...
*/
  ...
  */
  mutacion::mutacion(const string & str){
    ...
    /*--------------------------------*/ 
    this->pos=atoi(cadena[1].c_str()); // convierto el pos obtenido de string a numero entero y lo almaceno
    ...
    if(encuentra!=string::npos){
      ...
      dividir_string(copia, "|", cadena2);
      for(int i=0; i<cadena2.size(); i++){
        this->clnsig.push_back(atoi(cadena2[i].c_str()));
      }
    }
    else{
        for(int i=0; i<enfermedades.size(); i++){
          this->clnsig.push_back(atoi(copia.c_str()));
        }
    }
...
*/
  ...
  bool mutacion::esMenor(const string & chr, const unsigned int & pos) const{
    if(this->chr==chr){
      if(this->pos>pos){
        return false;
      }
